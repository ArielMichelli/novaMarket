...
export default function ProductList({ products, onAdd, loading, error }) {
  if (loading) return <div className="product-list"><h2>Productos</h2><p>Cargando...</p></div>
  if (error) return <div className="product-list"><h2>Productos</h2><p>Error cargando productos: {error}</p></div>
        ...
        {products.map((p) => (
          <li key={p.id} className="product-card">
            ...
            <div className="product-body">
              ...
              <p className="product-desc">{truncate(p.description, 120)}</p>
              <div className="product-actions">
                <button className="btn-primary" onClick={() => onAdd(p)}>Agregar al carrito</button>
              </div>
            </div>
          </li>
        ))}
